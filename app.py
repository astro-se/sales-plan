...
from src.io import load_sample_data, read_csv_upload, to_csv_bytes
...
with st.sidebar:
    ...
    use_sample = st.toggle("サンプルデータで確認", value=True)
    st.divider()
    st.caption("本番ではGoogle Sheets連携に差し替え可能。まずはCSVで同じ列構造を固定する。")
...
missing = [k for k in ["product_master", "sales_history"] if data[k].empty]
if missing:
